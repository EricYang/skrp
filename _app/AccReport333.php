<?php
    ...
    //header('Content-Type:text/plain; charset=utf-8');
    $ts_sql0 = "SELECT a.cDate, a.serialNo, c.cName AS staffName, b.accIocName, d.unitName, a.eDate, a.cAmounts
                FROM AccRequestPayment a INNER JOIN
                AccIoc b ON a.accIocSn = b.sn INNER JOIN
                Staff c ON a.staffSn = c.sn INNER JOIN
                PublicDept d ON a.publicDeptSn = d.sn
                WHERE (a.accType = 1) AND (d.infoCategory = 4) 
                AND a.cAmounts > a.amounts
    ...
    if ($ta_row0 != null){
        ...
        for ($p = 0; $p < count($ta_row0); $p++){
            ...
            $textConMulti = $textConMulti."<tr align='left'>
                ...
                <td align=\"left\">".$t_row0["eDate"]."</td>
                <td align=\"right\">".number_format($t_row0["cAmounts"], 2)."</td></tr>";
        }
      ...
      <div align="center">            
        ...
        <table border="1" cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#B4B4B4" width="100%" bgcolor="#F5F5F5">
          <tr>
            ...
            <td align="center" bgcolor="#FFFFCA"><font size="10"><b>預定支付日期</b></font></td>
